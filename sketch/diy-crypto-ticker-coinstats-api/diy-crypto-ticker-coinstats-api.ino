...
const long DEFAULT_DELAY = 5000; // wait 5 secionds between API request;

const int JSON_DOCUMENT_SIZE = 1536; //calculated by ArduinoJson calculator

// network
WiFiClientSecure client;

const char* HOST = "api.coinstats.app";
...
bool getInfo() {
  ...
  symbol = getSymbol();
  String endpoint = "/public/v1/coins/" + symbol + "?currency=" + CONVERT_TO;
  ...
  */
  ...
  JsonObject coin = root["coin"];
  name = coin["symbol"].as<String>();
  price = coin["price"];
  percent24h = coin["priceChange1d"];
  volume = coin["volume"];
  cap = coin["marketCap"];
...
void showInfo() {
  ...
  do {
    ...
    if (symbol == "vvs-finance")
      priceFormatted = String(price, 7);
    else
      priceFormatted = String(price, 2);
